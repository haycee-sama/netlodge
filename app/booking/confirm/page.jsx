...
// Booking Confirmation — upgraded with:
// 1. "What is escrow?" link opens EscrowExplainerModal
// 2. Sticky mini booking bar appears when price heading scrolls out of view
// 3. All original flow and fields preserved
...
import {
  ...
  ChevronLeft, ArrowRight, CheckCircle, AlertCircle, Lock,
} from 'lucide-react'
...
function BookingConfirmInner() {
  ...
  // Show sticky mini bar when price section scrolls out of view
  useEffect(() => {
  ...
  if (!result) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <p className="text-gray-500 text-sm">Please go back and select a room before proceeding.</p>
  ...
  function handleProceed() {
    ...
    if (hasError) return
    router.push(`/booking/pay?roomId=${roomId}&lease=${encodeURIComponent(leaseType)}&moveIn=${moveInDate}`)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {showEscrowModal && (
        <EscrowExplainerModal onClose={() => setShowEscrowModal(false)} />
      )}

      {/* ── Sticky mini booking bar ── */}
      {showMiniBar && (
        <div className="mini-booking-bar fixed top-0 left-0 right-0 z-40 bg-white border-b border-gray-100 shadow-sm">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div>
                <p className="text-xs text-gray-500">Room {room.number} — {room.type}</p>
                <p className="text-sm font-bold text-gray-900">{property.name} · {block.name}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right hidden sm:block">
                <p className="text-xs text-gray-400">Total</p>
                <p className="text-sm font-bold text-orange-500">₦{total.toLocaleString()}</p>
              </div>
              <button
                ...
                <div key={step} className="flex items-center gap-2">
                  <div className="flex items-center gap-2">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                      ...
                      {isActive ? index + 1 : <CheckCircle className="w-4 h-4" />}
                    </div>
                    <span className={`text-sm font-medium hidden sm:block ${isActive ? 'text-orange-500' : 'text-gray-400'}`}>
                      {step}
                    </span>
                  </div>
                  {index < 2 && <div className="h-0.5 w-8 sm:w-16 bg-gray-100" />}
                </div>
              )
            })}
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link href={`/rooms/${roomId}`} className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-orange-500 transition-colors mb-6">
          <ChevronLeft className="w-4 h-4" /> Back to Room Details
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">To change duration go back to the room page</p>
                </div>
                <div>
                  ...
                  <input
                    type="date" min={getTodayString()} value={moveInDate}
              ...
              {moveInDate && (
                <div className="mt-4 flex items-center gap-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-800">{getLeaseEndDate(moveInDate, leaseType)}</p>
                  </div>
                </div>
              )}
            </div>

            {/* Escrow Notice — with "What is escrow?" link */}
            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6">
              <div className="flex items-start gap-3">
                <Lock className="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <p className="font-semibold text-blue-900">Your Payment is Protected by Escrow</p>
                    {/* Escrow explainer trigger */}
                    <button
                      onClick={() => setShowEscrowModal(true)}
                      className="text-xs text-blue-600 underline hover:text-blue-800 font-medium shrink-0"
                    >
                      What is escrow?
                    </button>
                  </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 flex flex-col gap-4">
              <div ref={priceRef} className="bg-white rounded-2xl border border-gray-100 shadow-md p-6">
                ...
                <div className="bg-gray-50 rounded-xl p-3 mb-5 flex flex-col gap-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-gray-700">{moveInDate ? formatDate(moveInDate) : 'Not selected'}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">Lease ends</span>
                    <span className="font-semibold text-gray-700">{moveInDate ? getLeaseEndDate(moveInDate, leaseType) : '—'}</span>
                  </div>
                </div>
                <button
                  onClick={handleProceed}
