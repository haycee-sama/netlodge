// app/(public)/search/SearchClient.jsx
'use client'
...
export default function SearchClient({ properties }) {
  ...
  const hasActiveFilters =
    searchQuery !== '' || selectedCity !== 'All Cities' || selectedType !== 'All Types' ||
    selectedUni !== 'All Universities' || maxBudget < 250000
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-gray-500 text-sm">{filtered.length} verified properties available</p>
        </div>
      </div>
            ...
            <div className={`${showFilters ? 'block' : 'hidden'} lg:block`}>
              <div className="bg-white rounded-2xl border border-gray-100 p-6 flex flex-col gap-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {hasActiveFilters && (
                    <button onClick={clearFilters} className="text-xs text-orange-500 font-medium hover:text-orange-600 flex items-center gap-1">
                    ...
                    {CITIES.map((city) => (
                      <button
                        ...
                        className={`text-left text-sm px-3 py-2 rounded-lg transition-colors ${
                          selectedCity === city ? 'bg-orange-50 text-orange-600 font-medium' : 'text-gray-600 hover:bg-gray-50'
                        }`}
                      >
                        {city}
                      </button>
                  ...
                  <div className="flex flex-col gap-2">
                    {ROOM_TYPES.map((type) => (
                      <button
                        ...
                        className={`text-left text-sm px-3 py-2 rounded-lg transition-colors ${
                          selectedType === type ? 'bg-orange-50 text-orange-600 font-medium' : 'text-gray-600 hover:bg-gray-50'
                        }`}
                      >
                        {type}
                      </button>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-semibold text-orange-500">₦{maxBudget.toLocaleString()}</span>
                  </div>
                  <input
                    type="range"
                    min={50000}
          ...
          <div className="flex-1">
            ...
            {filtered.length > 0 ? (
              ...
              >
                {filtered.map((property) => (
                  ...
                  >
                    ...
                    >
                      <div className="relative sm:w-56 h-48 sm:h-auto shrink-0 bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center overflow-hidden">
                        {property.thumbnail ? (
                          <Image
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2 mb-4">
                            {property.roomTypes.map((type) => (
                              <span key={type} className="text-xs bg-orange-50 text-orange-600 font-medium px-2.5 py-1 rounded-full">
                                {type}
                              </span>
                            ))}
                          </div>
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-gray-500 text-sm mb-6 max-w-xs">Try adjusting your filters or search term.</p>
                <button onClick={clearFilters} className="bg-orange-500 text-white font-semibold px-6 py-2.5 rounded-xl hover:bg-orange-600 transition-colors text-sm">
                  Clear All Filters
                </button>
              </div>
            )}
