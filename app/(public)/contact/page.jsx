...
import {
  Mail, Phone, MapPin, Clock, ShieldCheck, GraduationCap, Building2, Send, CheckCircle, AlertCircle,
} from 'lucide-react'
import { submitContactForm } from '../../../lib/actions/contact'

const CONTACT_DETAILS = [
  { icon: Mail, label: 'Email', value: 'hello@netlodge.ng', sub: 'We reply within 4 hours on weekdays' },
  { icon: Phone, label: 'Phone', value: '+234 800 000 0000', sub: 'Monday to Saturday, 8am – 8pm WAT' },
  { icon: MapPin, label: 'Launch Cities', value: 'Abuja · Lagos · Enugu', sub: 'National expansion coming Year 2' },
  { icon: Clock, label: 'Support Hours', value: 'Mon – Sat: 8am to 8pm', sub: 'Sundays: Emergency support only' },
]
      ...
      <section className="bg-gray-900 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-sm font-medium text-orange-400 uppercase tracking-wider">Support</span>
          </div>
          <h1 className="text-4xl sm:text-5xl font-bold mb-4">We Are Here to Help</h1>
          <p className="text-gray-300 max-w-xl mx-auto">
            Whether you are a student with a booking question or a landlord with a KYC issue — our team responds within 4 hours on weekdays.
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="flex flex-col gap-6">
            <div>
              ...
              <p className="text-sm text-gray-500">Reach us through any of the channels below.</p>
            </div>

            {CONTACT_DETAILS.map((item) => {
              const Icon = item.icon
              return (
                <div key={item.label} className="bg-white rounded-xl border border-gray-100 p-4 flex items-start gap-4">
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-xl p-5">
              ...
              <div className="flex flex-col gap-2">
                <a href="/faq" className="flex items-center gap-2 text-sm text-orange-600 hover:underline">
                  <GraduationCap className="w-4 h-4" /> Student FAQs
                </a>
                <a href="/faq" className="flex items-center gap-2 text-sm text-orange-600 hover:underline">
                  <Building2 className="w-4 h-4" /> Landlord FAQs
                </a>
            ...
            {submitted ? (
              <div className="bg-white rounded-2xl border border-gray-100 p-12 flex flex-col items-center text-center">
                ...
                <p className="text-gray-500 max-w-md leading-relaxed mb-8">
                  Thank you for reaching out. Our support team will review your message and respond to <strong>{form.email}</strong> within 4 hours on weekdays.
                </p>
                <button
                  onClick={() => {
                    setSubmitted(false)
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-8">
                ...
                <p className="text-sm text-gray-500 mb-8">Fill in the form below and we will get back to you as soon as possible.</p>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 rounded-xl border text-sm text-gray-800 placeholder:text-gray-500 focus:outline-none focus:ring-2 transition-all ${
                          errors.name ? 'border-red-300 focus:ring-red-100' : 'border-gray-200 focus:ring-orange-100 focus:border-orange-400'
                        }`}
                      />
                  ...
                  <div>
                    ...
                    <select
                      name="queryType" value={form.queryType} onChange={handleChange}
                      className={`w-full px-4 py-3 rounded-xl border text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 transition-all ${
                        errors.queryType ? 'border-red-300 focus:ring-red-100' : 'border-gray-200 focus:ring-orange-100 focus:border-orange-400'
                      }`}
                    >
                      {QUERY_TYPES.map((qt) => (
                        <option key={qt.value} value={qt.value} disabled={qt.value === ''}>{qt.label}</option>
                      ))}
                    </select>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className={`w-full px-4 py-3 rounded-xl border text-sm text-gray-800 placeholder:text-gray-500 focus:outline-none focus:ring-2 transition-all resize-none ${
                        errors.message ? 'border-red-300 focus:ring-red-100' : 'border-gray-200 focus:ring-orange-100 focus:border-orange-400'
                      }`}
                    />
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 active:scale-[0.98] text-white font-bold py-4 rounded-xl transition-all text-base"
                  >
                    {loading ? (
                  ...
                  <p className="text-xs text-center text-gray-500">
                    We typically respond within 4 hours on weekdays. For urgent payment issues call us directly.
                  </p>
