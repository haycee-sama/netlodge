// app/(auth)/signup/landlord/page.jsx
'use client'

import { useState } from 'react'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import { Eye, EyeOff, ArrowRight, Building2, CheckCircle, AlertCircle } from 'lucide-react'
import AuthLayout from '../../../components/AuthLayout'
import { signupLandlord } from '../../../../lib/actions/auth'
...
export default function LandlordSignUpPage() {
  ...
  const [form, setForm] = useState({
    ...
    businessName: '', password: '', confirmPassword: '',
  })
  ...
  function validateClientSide() {
    ...
    if (form.password.length < 8 && form.password) e.password = 'Minimum 8 characters'
  ...
  async function handleSubmit(e) {
    ...
    const result = await signupLandlord(form)
    ...
    router.push(`/verify/email?email=${encodeURIComponent(form.email.trim().toLowerCase())}&role=landlord`)
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div key={field}>
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl border text-sm text-gray-800 placeholder:text-gray-500 focus:outline-none focus:ring-2 transition-all ${
                  errors[field] ? 'border-red-300 focus:ring-red-100' : 'border-gray-200 focus:ring-orange-100 focus:border-orange-400'
                }`}
              />
        ...
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-3 rounded-xl border text-sm text-gray-800 placeholder:text-gray-500 focus:outline-none focus:ring-2 transition-all ${
              errors.phone ? 'border-red-300 focus:ring-red-100' : 'border-gray-200 focus:ring-orange-100 focus:border-orange-400'
            }`}
          />
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3 pr-12 rounded-xl border text-sm text-gray-800 placeholder:text-gray-500 focus:outline-none focus:ring-2 transition-all ${
                  errors[field.name] ? 'border-red-300 focus:ring-red-100' : 'border-gray-200 focus:ring-orange-100 focus:border-orange-400'
                }`}
              />
              <button
                type="button"
        ...
        <p className="text-center text-xs text-gray-500">
          ...
          <Link href="/signup/student" className="text-gray-600 hover:underline">Sign up as a student</Link>
        </p>
