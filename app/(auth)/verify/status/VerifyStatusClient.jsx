// app/(auth)/verify/status/VerifyStatusClient.jsx
'use client'

import Link from 'next/link'
import {
  Clock, CheckCircle, XCircle, ShieldCheck, RefreshCw, Mail,
} from 'lucide-react'
...
export default function VerifyStatusClient({ status }) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center py-12 px-4">
      <div className="w-full max-w-lg">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center mb-5">
          ...
          {status === 'approved' && (
            <Link href="/dashboard" className="w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-xl transition-colors">
              Go to My Dashboard
            </Link>
          )}

          {status === 'rejected' && (
            <Link href="/verify/student" className="w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-xl transition-colors">
          ...
          {status === 'pending' && (
            <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4 text-left">
              ...
              <p className="text-sm text-blue-700">We will send an email and SMS to notify you the moment your verification is complete.</p>
            </div>
          )}
        </div>
